...
function TravelPlansDupe() {
  ...
  const submit_modify_travel_plan = () => {
    //this is wrong it should be 1 month less
    console.log(
      "updateTravelPlan",
      moment(updateTravelPlan.date[0]) //this part is ok
    );
    //this is correct
    console.log(
      "updateTravelPlanModal",
      moment(updateTravelPlanModal[0]) //this part is ok
    );

    //this is correct
    console.log(
      "1",
      moment(updateTravelPlanModal[0]).toDate() //this part is ok
    );

    //this is correct as well
    console.log(
      "2",
      moment(moment(updateTravelPlanModal[0]).toDate()).format("YYYY-MM-DD") //this part is ok
    );

    console.log(
      "startDate",
      moment(moment(updateTravelPlan.date[0]).toDate()).format("YYYY-MM-DD")
    );

    console.log(
      "endDate",
      moment(moment(updateTravelPlan.date[1]).toDate()).format("YYYY-MM-DD")
    );
    //this is okay also
    const formattedTravelPlan = {
      uuid: updateTravelPlan.uuid,
      name: updateTravelPlan.name,
      startDate: moment(moment(updateTravelPlan.date[0]).toDate()).format(
        "YYYY-MM-DD"
      ),
      endDate: moment(moment(updateTravelPlan.date[1]).toDate()).format(
        "YYYY-MM-DD"
      ),
    };
...
export default TravelPlansDupe;
